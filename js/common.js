...
---------------------------------------------------*/
	...
	window.ScrollEffects = function() {
		...
		function calculatePixels() {
			...
			var bgColor = document.getElementById('Gemicates-page-content').getAttribute('data-bgcolor');
		...
		if (isMobile()) {
			...
			function setViewPortHeight() {
				...
				gsap.set(".smooth-scroll main, .has-parallax, .Gemicates-nav-wrapper, .Gemicates-slider-wrapper:not(.content-slider), .showcase-lists .Gemicates-sync-slider, .slider-fixed-content", { height: winHeight});
			}
		...
		if ($('body').hasClass("swap-logo")) {
			...
			ScrollTrigger.create({
				trigger: '.Gemicates-header', // Set the trigger element
				start: 'top 120px',
				onEnter: function() {
				...
				onEnterBack: function() {
					...
					$('.Gemicates-header').removeClass('swapped-logo');
				},
				onLeave: function() {
					...
					$('.Gemicates-header').addClass('swapped-logo');			
				},
		...
		gsap.utils.toArray('.clipped-image-wrapper').forEach((clippedImageWrapper) => {
			...
			function setClippedImageWrapperProperties() {
				...
				gsap.set(clippedImage, { height: window.innerHeight - document.querySelector('.Gemicates-header').offsetHeight * 2 });								
				gsap.set(clippedImageContent, { paddingTop: (window.innerHeight/2) + clippedImageContent.offsetHeight});
				gsap.set(clippedImageWrapper, { height: window.innerHeight + clippedImageContent.offsetHeight - document.querySelector('.Gemicates-header').offsetHeight});
		...
		function setMainPageContentMargins() {
			var winHeight = $(window).height();
			var footerHeight = $('.Gemicates-footer').height();
		...
		gsap.to('.page-nav-caption', {
			...
			scrollTrigger: {
				trigger: '#page-nav',
				start: startValue,
				end: () => `+=${$('#page-nav').outerHeight() + $('.Gemicates-footer').outerHeight()}`,
				scrub: true,
			}			
		});
			...
			imagesLoaded('body', function() {
				setTimeout(function() {
					...
					changeHeaderColor.forEach(function(changeHeaderElement) {						
						var pageHeader = document.querySelector('.Gemicates-header');							
		...
		if ($('#Gemicates-page-content').hasClass("light-content")) {
			if ($('#project-nav').hasClass("change-header")) {
		  		imagesLoaded('body', function() {
					setTimeout(function() {	
						var pageNav = document.querySelector('#project-nav.change-header');
						if (pageNav) {
							var pageContent = document.querySelector('#Gemicates-page-content');	
		...
		if ($('#Gemicates-page-content').hasClass("dark-content")) {
			if ($('#project-nav').hasClass("change-header")) {
		  		imagesLoaded('body', function() {
					setTimeout(function() {	
						var pageNav = document.querySelector('#project-nav.change-header');
		...
		gsap.set('.header-gradient', {
			delay: 0.5,
			backgroundColor: document.getElementById('Gemicates-page-content').getAttribute('data-bgcolor'),			
			onUpdate: function() {
			var bgColor = document.getElementById('Gemicates-page-content').getAttribute('data-bgcolor');
		...
		$('a.ajax-link').on('click', function() {
			...
			gsap.to('#Gemicates-logo img.white-logo, #Gemicates-logo img.black-logo', { duration: 0.5, delay:0, opacity: "",  ease: Linear.easeNone, });
		...
		$("#hero #hero-caption.landing-page-title .inner").mouseenter(function(e) {    
			...
			$("#ball").append('<img class="eyes-icon" src="images/logo_white.png" style="transform:scale(0.75);">');
		
			// Funcție pentru schimbarea imaginilor între cele trei
			let imageIndex = 0;
			const images = [
				'<img class="eyes-icon" src="images/logo_white.png" style="transform:scale(0.75);">',
				'<img class="eyes-icon" src="images/logo_white.png" style="transform:scale(0.75);">'  // A treia imagine
			];
...
---------------------------------------------------*/	
	...
	window.FirstLoad = function() {
		...
		if ($("#Gemicates-page-content").hasClass("light-content")) {
			$(".Gemicates-nav-wrapper").css('background-color', function () {
				return $(".Gemicates-header").data('menucolor')
			});
			
			gsap.to("main", {duration: 0.5, backgroundColor: document.querySelector("#Gemicates-page-content").getAttribute("data-bgcolor"), ease:Power2.easeInOut});
			
			$('#magic-cursor').addClass('light-content');
			if( $('#hero').length > 0 ){						
				if( $('#hero').hasClass("has-image")) {	
					$(".Gemicates-header").css('background-color', 'transparent');
				} else {
					if ($(".Gemicates-header").hasClass("fullscreen-menu")) {
						$(".Gemicates-header").css('background-color', 'transparent');
					} else {
						if( $('#blog').length > 0 ){
							$(".Gemicates-header").css('background-color', '#171717');
						}
						if( $('#post').length > 0 ){
							$(".Gemicates-header").css('background-color', '#171717');
						}
			...
			if( $('#hero').length > 0 ){	
				if( $('#hero').hasClass("has-image")) {	
					...
					if ($(".Gemicates-header").hasClass("fullscreen-menu")) {
						...
						if( $('#blog').length > 0 ){
							$(".Gemicates-header").css('background-color', '#fff');
						}
						if( $('#post').length > 0 ){
							$(".Gemicates-header").css('background-color', '#fff');
						}
		...
		$('a.ajax-link').on('click', function() {
			...
			gsap.to("nav", {duration: 0.3, backgroundColor: document.querySelector("#Gemicates-page-content").getAttribute("data-bgcolor")});
		...
		$('.Gemicates-nav-wrapper .ajax-link').on('click', function() {
		...
		$('#burger-wrapper, .menu .button-text').on('click', function() {
			$('#menu-burger, .Gemicates-nav-wrapper').toggleClass('open');			
			setTimeout( function(){			
				if ($('#menu-burger').hasClass("open")) {
					
					gsap.to('.Gemicates-nav-wrapper', {duration: 0.3, opacity:1, ease:Power2.easeInOut});
					
					$('.Gemicates-header').addClass('over-sidebar').addClass('over-white-section');
					if (!$('#page-content').hasClass("light-content")) {	
						$('#magic-cursor').addClass('light-content');
					}
					if ($('.Gemicates-header').hasClass("invert-header")) {
						$('#header-container').addClass('light-content-header');
					} else {
						$('#header-container').addClass('dark-content-header');
					}
					gsap.set($(".Gemicates-nav-wrapper ul ul li"), {y: 0, opacity:1});
					...
					gsap.to('.Gemicates-nav-wrapper', {duration: 0.3, opacity:0, delay:0.6, ease:Power2.easeInOut});
						
					//Fade Out Navigation Lists						
					gsap.to($(".menu-timeline .before-span"), {duration: 0.5, y:-200, opacity:1, delay:0, stagger:0.05, ease:Power2.easeIn});
					gsap.to($(".Gemicates-nav-wrapper ul ul li"), {duration: 0.5, y:-120, opacity:0, delay:0, stagger:0.03, ease:Power2.easeIn});
					...
					setTimeout( function(){
						...
						$('.Gemicates-header').removeClass('over-sidebar')
						setTimeout( function(){
							$('.Gemicates-header').removeClass('over-white-section');
						} , 350 );
					} , 500 );
				}							
			} , 20 );
				  ...
				  $body.on('mouseup mousemove', function handler(evt) {
					if (evt.type === 'mouseup') {
					  ...
					  $('#itemsWrapperLinks .trigger-item-link, #itemsWrapperLinks .trigger-item-link-secondary').on('click', function() {					
							...
							/*if (!$('#Gemicates-page-content').hasClass("light-content")) {
								...
								if (!$('.portfolio').hasClass("portfolio-shortcode")) {
									if (!parent_item.hasClass("change-header")) {
										$('#Gemicates-page-content').delay(700).queue(function(next){							
							...
							$('.Gemicates-slider-wrapper .trigger-item').each(function(){
							...
							gsap.to('#hero, #show-filters, .item-caption-wrapper, .showcase-portfolio .slide-caption, #page-nav, .Gemicates-footer, .fadeout-element', {duration: 0.5, opacity:0, ease:Power4.easeInOut});
...
---------------------------------------------------*/
	...
	window.Sliders = function() { 
		...
		if( $('.content-slider').length > 0 ){
			...
			slider = new GemicatesSlider('.content-slider', { 
			...
			$(".slider-button-prev").mouseenter(function(e) {				
				...
				if (modifyMouseColor) {
					...
					if ($(this).parents(".Gemicates-slider-wrapper").hasClass("light-cursor")) {
						$("body").addClass("drag-cursor-white");
						gsap.to('#ball', {duration: 0.2, borderWidth: '2px', scale: 1, borderColor:'#fff', backgroundColor:'#fff'});
					} else if ($(this).parents(".Gemicates-slider-wrapper").hasClass("dark-cursor")) {
			...
			$('.content-slider.looped-carousel .Gemicates-slider, .content-slider.small-looped-carousel .Gemicates-slider').on('mouseenter mousemove', function() {	
...
---------------------------------------------------*/
	...
	window.Lightbox = function() {
		...
		function showDetails(item) {
			...
			$('body').prepend(`<div class="Gemicates-img-popup">
				<div class="Gemicates-img-popup-bg-close"></div>				
				<div class="Gemicates-img-popup-viewport">
					<div class="Gemicates-img-popup-preloader"><div></div><div></div><div></div><div></div></div>					
					<img />
					<div class="Gemicates-img-popup-close link"></div>						
				</div>				
				<div class="Gemicates-img-popup-prev link"></div>
				<div class="Gemicates-img-popup-next link"></div></div>`);
			
			const details = document.querySelector('.Gemicates-img-popup');
			const detailsBgClose = document.querySelector('.Gemicates-img-popup-bg-close');
			const detailsClose = document.querySelector('.Gemicates-img-popup-close');
			const detailsPreloader = document.querySelector('.Gemicates-img-popup-preloader');
			const detailImage = document.querySelector('.Gemicates-img-popup img');
			const detailPrev = document.querySelector('.Gemicates-img-popup-prev');
			const detailNext = document.querySelector('.Gemicates-img-popup-next');
		...
		function showVideoDetails(event, item) {
			...
			$('body').prepend(`<div class="Gemicates-video-popup">
				<div class="Gemicates-video-popup-bg-close"></div>				
				<div class="Gemicates-video-popup-viewport">
					<div class="Gemicates-video-popup-preloader"><div></div><div></div><div></div><div></div></div>					
					<iframe class="Gemicates-video-popup-iframe" frameborder="0" allow="autoplay"></iframe>
					<div class="Gemicates-video-popup-close link"></div>						
				</div>				
				<div class="Gemicates-img-video-prev link"></div>
				<div class="Gemicates-img-video-next link"></div></div>`);
			
			const details = document.querySelector('.Gemicates-video-popup');
			const detailsBgClose = document.querySelector('.Gemicates-video-popup-bg-close');
			const detailsClose = document.querySelector('.Gemicates-video-popup-close');
			const detailsPreloader = document.querySelector('.Gemicates-video-popup-preloader');
			const detailIframe = document.querySelector('.Gemicates-video-popup iframe');
...
---------------------------------------------------*/
	...
	window.Core = function() {
		...
		if (!isMobile() && !$('body').hasClass("disable-cursor")) {
			...
			$(".Gemicates-button .sticky.left").mouseenter(function(e) {		  
				var rcBounds = $(this)[0].getBoundingClientRect();		  
			...
			$(".Gemicates-button .sticky.right").mouseenter(function(e) {		  
				var rcBounds = $(this)[0].getBoundingClientRect();		  
			...
			$(".Gemicates-button .parallax-wrap.icon-wrap").mouseenter(function(e) {
			...
			$(".Gemicates-button .sticky").mouseenter(function(e) {
				if ($('#Gemicates-page-content').hasClass("light-content")) {
			...
			$(".Gemicates-button .parallax-wrap").mouseenter(function(e) {
				if ($('#Gemicates-page-content').hasClass("light-content")) {
			...
			//Blog Hover Effects			
			$("#blog-page-nav .page-numbers li a, .post-page-numbers, #post-content a, #post-form a, #post-comments a, .wp-block-search__button, .Gemicates-sidebar-widget a").mouseenter(function(e) {	
				gsap.to('#ball', {duration: 0.2, borderWidth: '1px', scale: 1, opacity:0});
			});	
					
			$("#blog-page-nav .page-numbers li a, .post-page-numbers, #post-content a, #post-form a, #post-comments a, .wp-block-search__button, .Gemicates-sidebar-widget a").mouseleave(function(e) {
				gsap.to('#ball', {duration: 0.3, borderWidth: '4px', scale:0.5, opacity:1});
			});
		}
			  ...
			  $(window).on('popstate', function() {
				...
				section.load(url+' .cd-main-content > *', function(event){
				  // load new content and replace <main> content with the new one
				 	...
				 	var Gemicates_title = event.match(/<title[^>]*>([^<]+)<\/title>/)[1];
					$('head title').html( Gemicates_title );
