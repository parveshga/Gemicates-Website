...
---------------------------------------------------*/			
	...
	function InitShuffleElements() {
		...
		shuffleFunctions.applyShuffleEffect(".showcase-portfolio .Gemicates-item .slide-inner", {
			velocity: 30,
			shuffleIterations: 8,
			childSelector: '.slide-title span, .slide-cat span'
		});
		
		shuffleFunctions.applyShuffleEffect(".Gemicates-slide .slide-inner-height", {
			velocity: 30,
			shuffleIterations: 8,
			childSelector: '.slide-title span, .slide-cat span'
		});
...
---------------------------------------------------*/	
	...
	function CleanupBeforeAjax(){		
		...
		GemicatesSlider.instances.forEach(slider => slider.off());
		GemicatesSlider.instances = [];
	}
...
---------------------------------------------------*/
	...
	function PageLoad() {		
		...
		gsap.set($(".Gemicates-header"), {yPercent:-50, opacity:0});
		...
		function initOnFirstLoad() {
			...
			imagesLoaded('body', function() {
				...
				gsap.to($(".Gemicates-header"), { duration: 0.45, opacity:1, yPercent:0, delay:0.25, ease: Power2.easeOut, });
	...
	function PageLoadActions() {
		...
		if (!isMobile()) {
			...
			$("#page-nav .next-ajax-link-page").on('mouseenter', function() {
				var modifyMouseColor = $('#Gemicates-page-content').attr("data-modify-color");
				if (modifyMouseColor) {
		...
		if (!$("body").hasClass("disable-ajaxload")) {
			...
			$('#page-nav .next-ajax-link-page').on('click', function() {	
				...
				$('.Gemicates-header').removeClass('white-header');
				$("#app").remove();
				...
				gsap.to($("#main-page-content, #hero, .Gemicates-footer, .Gemicates-footer-nav"), {duration: 0.3, opacity:0, ease:Power4.easeOut});
				
				if ($("#page-nav").hasClass("move-nav-onload")) {
					$("body").addClass("load-next-page");
					if ($("body").hasClass("smooth-scroll")) {
						var moveNav = $("#content-scroll").height() - ( $("#hero").height() / 2 )  - ( $("#page-nav").height()  ) / 2 - $(".Gemicates-footer").height() / 2	
					} else {
						var moveNav = window.innerHeight - ( $("#hero").height() / 2 )  - ( $("#page-nav").height()  ) / 2 - $(".Gemicates-footer").height() / 2	   
					}
			...
			$('#page-nav .next-ajax-link-page').on('click', function() {					
				...
				gsap.to($(".Gemicates-footer, .Gemicates-footer-nav"), {duration: 0.3, opacity:0, delay:0, ease:Power2.easeInOut});
			});
		...
		if (!$("body").hasClass("disable-ajaxload")) {
			...
			$('.next-ajax-link-project').on('click', function() {
				...
				if ($(".Gemicates-header").hasClass("swapped-logo")) {
					...
					gsap.to($("#Gemicates-logo"), {duration: 0.2, opacity:0, onComplete: function() {
						imgLogoWhite.src = originalSrcWhite;
						imgLogoBlack.src = originalSrcBlack;
						gsap.to($("#Gemicates-logo"), {duration: 0.2, opacity:1});
					}});
				...
				gsap.to($(".Gemicates-footer, .all-works"), {duration: 0.3, opacity:0, ease:Power2.easeInOut});
		...
		if( $('#project-nav').length > 0 ){		
			...
			const pageContent = document.getElementById("Gemicates-page-content");
			...
			ScrollTrigger.create({
				...
				onLeaveBack: function() {
					...
					gsap.set("#Gemicates-logo img, .classic-menu .flexnav li a, .button-wrap.menu, .button-icon-link", { clearProps: "all" });
				},			
				scrub: true,
			});	
			...
			if ($("#project-nav").hasClass("change-header")) {
				...
				if (changeMainColor) {
					...
					imagesLoaded('body', function() {
						...
						const blackLogoOpacity = gsap.getProperty('#Gemicates-logo img.black-logo', 'opacity');
						const whiteLogoOpacity = gsap.getProperty('#Gemicates-logo img.white-logo', 'opacity');
						...
						ScrollTrigger.create({
							...
							onEnter: function( st ) {
								...
								gsap.fromTo('#Gemicates-logo img.black-logo', {
								...
								gsap.fromTo('#Gemicates-logo img.white-logo', {
									opacity: whiteLogoOpacity
								}, {
									duration: 1, 
									opacity: blackLogoOpacity,
...
---------------------------------------------------*/	
	...
	function ShowcasePortfolio() {
		...
		gsap.utils.toArray('.Gemicates-item .pixels-cover').forEach((pixelWrapper, index) => {
			const pixelAnimation = pixelWrapper.querySelectorAll(".pixel");
		...
		if( $('.showcase-portfolio').length > 0 ){
			...
			function filter() {				
				...
				var state = Flip.getState('.Gemicates-item');
				var projects = document.querySelectorAll('.Gemicates-item');
				var startHeight = gsap.getProperty(".showcase-portfolio", "height");
				
				var filters = document.querySelectorAll('.filter-option.is_active');
				var parallaxItems = document.querySelectorAll('.showcase-portfolio .Gemicates-item.vertical-parallax .slide-inner');
			...
			function resetItems() {
				$('.expand-grid .Gemicates-item').removeClass('expanded not-expanded').addClass('default');
			}
			...
			function expandItem($item) {
				...
				$('.expand-grid .Gemicates-item').slice(start, end).not($item).addClass('not-expanded').removeClass('default');
			}		
		
			expandItem($('.expand-grid .Gemicates-item').eq(1));
		
			$('.expand-grid .Gemicates-item').on('mouseenter', function() {
			...
			if( $('.filp-grid').length > 0 ) {
				...
				if (!isMobile()) {
					...
					const GemicatesItems = document.querySelectorAll('.showcase-portfolio .Gemicates-item');
					
					
					GemicatesItems.forEach(item => {
					...
					if (showcasePortfolio.classList.contains('filp-grid')) {
						...
						const GemicatesItems = showcasePortfolio.querySelectorAll('.Gemicates-item');						
						GemicatesItems.forEach(item => {
					...
					const overlappingThumbs = document.querySelectorAll('.showcase-portfolio .Gemicates-item .img-mask');
			...
			if (!isMobile()) {	
				...
				$(".showcase-portfolio .Gemicates-item .slide-inner").on('mouseenter', function() {
					$('#ball p').remove();
					var $this = $(this);			
...
---------------------------------------------------*/
	...
	function ShowcaseCarousel() {
		...
		if( $('.showcase-carousel').length > 0 ){
			...
			gsap.set($(".showcase-carousel .Gemicates-slider .slide-inner-height"), { scale: 0.8 });
				...
				setTimeout(function() {	
					...
					function animatePixels(selector, minDelay, maxDelay) {
						gsap.utils.toArray(selector).forEach((pixelWrapper) => {
							const pixelAnimation = pixelWrapper.querySelectorAll(".Gemicates-slider .pixel");
					...
					animatePixels('.Gemicates-slide-active .pixels-cover', 0, 0.3);
					animatePixels('.Gemicates-slide:not(.Gemicates-slide-active) .pixels-cover', 0.3, 0.6);
					
					gsap.to($(".showcase-carousel .Gemicates-slider .slide-inner-height"), { duration: 0.6, scale: 1, ease: Power2.easeOut });
			...
			slider = new GemicatesSlider('.showcase-carousel', { 
				direction: 'horizontal',
				ease: 0.065, 
				snap:false,
				outer: '.Gemicates-slider',
				inner: '.Gemicates-slider-viewport',
				navigation: {
				...
				on: {	
					...
					slideLeaveViewport : function( slide ) {
						gsap.set($('.Gemicates-slider div:not(.Gemicates-slide-visible) .slide-effects'), { y: "" });		
					},
				},
			});
			...
			if (!isMobile()) {
				...
				$('.Gemicates-slider').on('mousedown', function (evt) {
					$('.Gemicates-slider').on('mouseup mousemove', function handler(evt) {
						if (evt.type === 'mouseup') {					  
							// click
							gsap.to('#ball', {duration: 0.2, borderWidth: '4px', scale:0.5, borderColor:'#999999', backgroundColor:'transparent'});
						...
						$('.Gemicates-slider').off('mouseup mousemove', handler);
					});
				});
				
					
				$('.Gemicates-slider').on('mouseup touchend', function() {
				...
				$(".showcase-carousel .Gemicates-slide .slide-inner-height").on('mouseenter', function() {	
					if (!$('body').hasClass('scale-drag-x')) {
						...
						$('.Gemicates-slide').addClass('disable');						
						$(this).closest(".Gemicates-slide").removeClass("disable");
					}			
				}).on('mouseleave', function() {					
					if (!$('body').hasClass('scale-drag-x')) {
						gsap.to('#ball', {duration: 0.2, borderWidth: '4px', scale:0.5, borderColor:'#999999', backgroundColor:'transparent'});
						...
						$(this).closest(".Gemicates-slide").addClass("disable");
						$('.Gemicates-slide').removeClass('disable');	
					}
				});
...
---------------------------------------------------*/
	...
	function ShowcaseGallery() {
		...
		if( $('.showcase-gallery').length > 0 ){
			...
			if (isMobile()) {
				$(".Gemicates-slider-wrapper").removeClass("preview-mode-enabled");
			}
			
			var slideElements = $(".Gemicates-slide");
            ...
            $('#Gemicates-page-content').append(bgPixels);
			...
			gsap.set($(".showcase-gallery .Gemicates-slider .slide-inner-height"), { opacity: 0 });
			...
			if (!$('body').hasClass("show-loader")) {
				...
				setTimeout(function() {
					gsap.to($(".showcase-gallery .Gemicates-slider .Gemicates-slide .slide-inner-height"), { duration: 0.5, opacity: 1, delay: 0.2, ease: Power2.easeInOut });
					var gallerySlideClasses = [".Gemicates-slide-prev-two", ".Gemicates-slide-prev", ".Gemicates-slide-active", ".Gemicates-slide-next", ".Gemicates-slide-next-two"];
					gallerySlideClasses.forEach(function(gallerySlideClass, index) {
						var gallerySlide = $(".showcase-gallery .Gemicates-slider " + gallerySlideClass + " .slide-inner-height");
			...
			slider = new GemicatesSlider('.Gemicates-slider-wrapper', { 
				direction: 'vertical',
				ease: 0.065, 
				outer: '.Gemicates-slider',
				inner: '.Gemicates-slider-viewport',
					...
					init : function(slide) {
						...
						if ($('body').hasClass("show-loader")) {
							...
							imagesLoaded('body', function() {
								...
								gsap.to($(".showcase-gallery .Gemicates-slider .Gemicates-slide .slide-inner-height"), { duration: 0.5, opacity: 1, delay: 0.4, ease: Power2.easeInOut });
			...
			const showGrid = () => {
				...
				document.getElementById("Gemicates-page-content").classList.add('no-css-animation');
				...
				gsap.set(currentImage.closest(".Gemicates-slide"), {zIndex: 0});
				...
				if (document.getElementById('Gemicates-page-content').classList.contains('dark-content')) {
					if (currentImage.classList.contains('change-header')) {
						gsap.to('#Gemicates-logo img.black-logo', { duration: 0.5, delay:0.15, opacity: 0, ease:Power2.easeInOut });	
						gsap.to('#Gemicates-logo img.white-logo', { duration: 0.5, delay:0.15, opacity: 1, ease:Power2.easeInOut });						
				...
				} else if (document.getElementById('Gemicates-page-content').classList.contains('light-content')) {
					if (currentImage.classList.contains('change-header')) {
						gsap.to('#Gemicates-logo img.black-logo', { duration: 0.5, delay:0.15, opacity: 1, ease:Power2.easeInOut });	
						gsap.to('#Gemicates-logo img.white-logo', { duration: 0.5, delay:0.15, opacity: 0, ease:Power2.easeInOut });						
				...
				gsap.to(".Gemicates-slider .Gemicates-slide .trigger-item", { duration: 1, opacity:0, scale:0.7, ease:Power2.easeOut });
			...
			const hideGrid = () => {
				...
				gsap.to(".Gemicates-slider .Gemicates-slide .trigger-item", { duration: 0.5, opacity:1, scale:1, delay:0.2, ease: 'power3.inOut' });
				...
				Flip.from(flipstate1, {
					...
					onComplete: function() {
						...
						const triggeredItem = document.querySelector('.Gemicates-slide.triggered-item');
						if( triggeredItem != null ){							
							triggeredItem.classList.remove('triggered-item');
						}
						
						const GemicatesSlides = document.querySelectorAll('.Gemicates-slide');
						GemicatesSlides.forEach(slide => {
						  	slide.style.zIndex = '';
						...
						document.getElementById("Gemicates-page-content").classList.remove('no-css-animation');
						gsap.set($(".showcase-gallery .slide-caption"), {clearProps: "opacity"});
						
					}
				})								
				
				gsap.to('.header-gradient', { duration: 0.4, backgroundColor: $("#Gemicates-page-content").data("bgcolor"), ease:Power2.easeInOut });
				gsap.to("main", { duration: 0.4, backgroundColor: $("#Gemicates-page-content").data("bgcolor"), ease: Power2.easeInOut });
				gsap.to("#bg-pixels", { duration: 0.4, delay:0.15, opacity:0.75, ease: Power2.easeInOut });			
				
				
				if (document.getElementById('Gemicates-page-content').classList.contains('dark-content')) {
					if (currentImage.classList.contains('change-header')) {
						gsap.to('#Gemicates-logo img.black-logo', { duration: 0.4, opacity: 1, ease:Power2.easeInOut });	
						gsap.to('#Gemicates-logo img.white-logo', { duration: 0.4, opacity: 0, ease:Power2.easeInOut });						
					...
					if (currentImage.classList.contains('change-header')) {						
						gsap.to('#Gemicates-logo img.black-logo', { duration: 0.4, opacity: 0, ease:Power2.easeInOut });	
						gsap.to('#Gemicates-logo img.white-logo', { duration: 0.4, opacity: 1, ease:Power2.easeInOut });						
				...
				gsap.to($("footer .link-text, .Gemicates-pagination, #filters-wrapper"), {duration: 0.3, opacity:1, y:0, stagger:0.05, delay:0.4, ease:Power2.easeInOut});
				gsap.to('#ball', {duration: 0.2, borderWidth: '4px', scale:0.5, borderColor:'#999999', backgroundColor:'transparent'});
				gsap.to('#ball-loader', {duration: 0.2, borderWidth: '4px', top: 0, left: 0});
				$("#ball").removeClass("with-blur color-cursor");
				$('#ball p').remove();
			...
			const getDOMElements = () => {
				...
				const items = [];
				[...document.querySelectorAll('.Gemicates-slide')].forEach(item => {
					items.push(new Item(item));
				});
						
				// Cloned items
				const itemsCloned = [];
				[...document.querySelectorAll('.Gemicates-slide-clone')].forEach(item => {
				...
				return {					
					items: items,					
					grid: gridItems,					
					currentItem: new Item( document.querySelector('.Gemicates-slide.triggered-item') ),
				...
				items.forEach((triggerItem) => {
					...
					triggerItem.addEventListener('click', (event) => {
						...
						if( $('.showcase-gallery').length > 0 ){
							...
							event.currentTarget.closest('.Gemicates-slide').classList.add('triggered-item');
							showGrid();
						}
					});
			...
			if (!isMobile()) {
				...
				$(".showcase-gallery.preview-mode-enabled .Gemicates-slide .slide-inner-height").on('mouseenter', function() {	
			...
			var currentSlide = null;
				...
				$('.Gemicates-slide .slide-inner-height').on('mouseenter', function() {
					
					var slide = $(this).closest(".Gemicates-slide").data('slide');
		...
		if ($(".projects-list-wrapper").hasClass("invert-header-color")) {
			...
			changeMainColor.forEach(function(changeHeaderColor) {
				...
				imagesLoaded('body', function() {
					...
					const bgPageContent = document.getElementById("Gemicates-page-content");
					...
					const scrollTriggerEnter = {
						...
						onLeaveBack: function() {
							bgPageContent.classList.remove('no-css-animation');
							gsap.set('#Gemicates-logo img.black-logo, #Gemicates-logo img.white-logo', {opacity: "" });
						}
					};
					...
					$('.trigger-item').on('click', function() {
						...
						gsap.to('#Gemicates-logo img.white-logo, #Gemicates-logo img.black-logo', { duration: 0.5, delay:1, opacity: "",  ease: Linear.easeNone, });
